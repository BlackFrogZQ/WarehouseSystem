...
void drawBack(QPainter *painter, const QStyleOptionViewItem &option, const QRect &fillRect, CParaNodeType p_type, int p_col) //根据事件画背景
{
    ...
    painter->fillRect(fillRect, QColor(42, 57, 60)); //未选中
    ...
    {
        painter->fillRect(fillRect, QColor(181, 200, 206)); //选中
    ...
    {
        painter->fillRect(fillRect, QColor(69, 122, 95)); //鼠标移入
        assert(p_col == 1 || p_col == 0);
...
void drawText(QPainter *painter, const QStyleOptionViewItem &option, const CParaNodeAttributes &attributes, const QModelIndex &index)
{
    ...
    {
        QFont font = option.font;
        QColor textColor = QColor(181, 200, 206);
        if (option.state & QStyle::State_Selected)
        {
            font.setBold(true);
...
void drawGrid(QPainter *painter, const QStyleOptionViewItem &option, const CParaNodeType &p_type)
{
    int x = option.rect.x();
    int y = option.rect.y();
    int width = option.rect.width();
