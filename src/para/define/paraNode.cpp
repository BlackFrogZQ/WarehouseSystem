...
namespace TIGER_ParaDef
{
    ...
    void CParaNode::appendNode(const CParaNodeAttributes &p_attributes, void *p_pValue)
    {
        ...
        if (id >= 0)
        {
            myDebug << cnStr("发现相同子节点->名:%1,类型%2").arg(getNodePathName(m_childrenNode.at(id))).arg(p_attributes.type);
            return;
        }
    ...
    void CParaNode::setValue(QVariant p_value)
    {
        ...
        case pntEnum:
            ...
            {
                trace(cnStr("枚举参数值超过备注项数：name:'%1', cnName: '%2', value:'%3', remark:'%4'").arg(getNodePathName(this)).arg(m_attributes.cnName).arg(p_value.toUInt()).arg(m_attributes.remark));
                *((uint *)this->m_value) = 0;
                break;
            }
