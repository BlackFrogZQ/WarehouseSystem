...
namespace TIGER_FlieTool
{
    ...
    bool saveStructToFile(const QString &p_file, const T &p_T)
    {
        ...
        {
            myDebug << cnStr("创建%1失败").arg(p_file);
            return false;
        }
        ...
        {
            myDebug << cnStr("保存%1失败:%2").arg(p_file).arg(file.errorString());
            return false;
        }
    ...
    bool loadStructFormFile(const QString &p_file, T &p_T)
    {
        if (!isFile(p_file))
        {
            myDebug << cnStr("加载%1文件失败:文件不存在").arg(p_file);
            return false;
        }
        QFile file(p_file);
        if (!file.open(QFile::ReadOnly))
        {
            myDebug << cnStr("打开%1文件失败:%2").arg(p_file).arg(file.errorString());
            return false;
        }
